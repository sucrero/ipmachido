        ...
        if($conexion->registros > 0){                           
            ...
            for($i = 0 ;$i < count($res);$i++){
                ...
                if($res[$i][0]['pais'] == '189'){
                    ...
                    $sql = "SELECT * FROM tbl_cne_municipio WHERE cod_municipio = '".$res[$i][0]['municipiodir']."'";
                ...
                if($pais != '0' ){
                    $tpais = $res[0][0]['nompais'];
                }else{
                    $tpais = "TODOS";
                }
                if($estado != '0'){
                    if($pais == '189'){
                        $testado = $res[0][4]['nomb_estado'];
                    }else{
                        $testado = $res[0][4]['nomestmundo'];
                    }
                }else{
                    $testado = "TODOS";
                }
                if($municipio != 0){
                    $tmunicipio = $res[0][5]['nomb_municipio'];
                }else{
                    $tmunicipio = "TODOS";
                }
        }else{
...
//    print_r($res); exit();
    $titulo = html_entity_decode("Pa&iacute;s: ".$tpais."   Estado: ".$testado."   Municipio: ".$tmunicipio,ENT_QUOTES,"ISO-8859-1")."  ".$conf;
    class PDF extends PDF_MC_Table{
        ...
        function Footer() {
            ...
            $this->Cell(60,4, 'Impreso por: '.$_SESSION['nombre'], 0, 0, 'C');
...
//                    $talla = $res[$i][0]['talla'];
                ...
                for($i = 0;$i < $num;$i++){                 
                    ...
                    $this->SetFont('Arial','',10);
                    $this->Cell(0, 5, html_entity_decode(utf8_decode($direccion),ENT_QUOTES,"ISO-8859-1"),1, 1, 'L');
                    $this->SetFont('Arial','B',10);
