...
$(function() {
    ...
    document.getElementById('itxtnombreusu').focus();
...
{ 
    ...
    {
        consultarPc($(this).data("row-id"));
    }).end().find(".command-delete").on("click", function(e)
    {
        eliminarPc($(this).data("row-id"));
    });
});
...
function numregistro(){
    var num = document.getElementById('numregis');
    $.ajax({
        url: "../Operaciones.php",
        type: "POST",
        data: {opcion: "buscarnumregis"},
        dataType: 'json',
...
function eliminarPc(id){
    if(confirm("Desea eliminar este regoistro?")){
        $.ajax({
            url: "../"+$('#formpc').attr("action"),
            type: $('#formpc').attr("method"),
            data: {opcion: "eliminarpc",id: id},
            dataType: 'json',
            complete: function (data) {
                var dt =  JSON.parse(data.responseText);
                if(dt == 1){
...
function modificarPc(){
    var mac = document.getElementById('itxtmac');
    var dpto = document.getElementById('itxtdepartamento');
    var ubi = document.getElementById('itxtubicacion');
    var nompc = document.getElementById('itxtnombrepc');
    var ip = document.getElementById('itxtip');
                ...
                if(mac.value != ''){
                    if (regex.test(mac.value)){
                        if(ip.value != ''){
                            ...
                            alert("Debe ingresar una direccion ip");
                            ip.focus();
                        }
                    }else{
                        alert("Direccion MAC inválida, verifique");
                        mac.focus();
                    }
                }else{
                    alert("Debe ingresar una direccion mac");
                    mac.focus();
                }
            }else{
                alert("Debe ingresar una numeracion para el nombre de la pc");
                nompc.focus();
            }
        }else{
...
function consultarPc(id){
    $.ajax({
        url: "../"+$('#formpc').attr("action"),
        type: $('#formpc').attr("method"),
        data: {opcion: "consultarpc",id: id},
        dataType: 'json',
        complete: function (data) {
            var dt =  JSON.parse(data.responseText);
            $('#numregis').val(dt[0][0]);
            $('#itxtnombreusu').val(dt[0][1]);
            $('#itxtnombrepc').val(dt[0][2]);
            var mac = dt[0][3].replace(/:/g,'');
            $('#itxtmac').val(mac);
            var ip = dt[0][4].split('.');
            ip = ip[3].split('/');
            $('#itxtip').val(ip[0]);
            $('#itxtubicacion').val(dt[0][5]);
            $('#itxtdepartamento').val(dt[0][9]);
            $('#itxtobservacion').val(dt[0][10]);
            $('#opcion').val('modificarpc');
